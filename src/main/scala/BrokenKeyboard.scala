  ...
  def possibleWords() {
    ...
    for (i <- 0 until availableWords.length) {
      ...
      while (lengthCount < word.length) {
        if (word.substring(lengthCount, lengthCount + 1) != availableLetters(0) && word.substring(lengthCount, lengthCount + 1) != availableLetters(1) && word.substring(lengthCount, lengthCount + 1) != availableLetters(2) && word.substring(lengthCount, lengthCount + 1) != availableLetters(3)) {
